...
namespace game_framework {
	...
	void CGameState::ShowInitProgress(int percent)
	{
		if (!SHOW_LOAD_PROGRESS)
			return;
		const int bar_width = SCREEN_SIZE_X * 2 / 3;
		const int bar_height = SCREEN_SIZE_Y / 20;
		const int x1 = (SCREEN_SIZE_X - bar_width) / 2;
		const int x2 = x1 + bar_width;
		const int y1 = (SCREEN_SIZE_Y - bar_height) / 2;
		...
		loading.SetTopLeft((SCREEN_SIZE_X - loading.Width()) / 2, y1 - 2 * loading.Height());
		loading.ShowBitmap();
		//
		// 以下為CDC的用法
		//
	...
	void CGame::OnInit()	// OnInit() 只在程式一開始時執行一次
	{
		...
		// 開啟DirectX繪圖介面
		//
		CDDraw::Init(SCREEN_SIZE_X, SCREEN_SIZE_Y);							// 設定遊戲解析度
		//
		// 開啟DirectX音效介面
		//
